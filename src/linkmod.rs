...
fn parse_json_into_modder(username: &str, json: &JsonValue) -> Modder {
    ...
    let most_popular_mod = mods.iter()
        .find(|a| {
            a.download_count == downloads_vec[downloads_vec.len() - 1]
        })
        .unwrap()
        .clone();
    ...
    // Find first mod
    let first_mod = mods.iter()
        .find(|a| a.creation_date == release_dates[0])
        .unwrap()
        .clone();

    // Find last updated mod
    let last_updated_mod = mods.iter()
        .find(|a| a.last_updated == update_dates[update_dates.len() - 1])
        .unwrap()
        .clone();
